   ...
   )
{
   ...
   DbgPrint("TLInspectALEConnectNotify");
      ...
      if (!IsListEmpty(&gConnList))
      {
              ...
              listEntry = listEntry->Flink)
         {
            ...
            if (packet->authConnectDecision == 0)
            {
				...
				status = AVCommSendEvent(&eventPacket,
					sizeof(AV_EVENT_NETWORK_PACKET),
					&UMResponse,
					&replyLength);
