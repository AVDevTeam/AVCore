   ...
   )
{
   ...
   DbgPrint("AVNetwork || Debug || WdfRegistryQueryUnicodeString: %x\n", status);
   ...
   {  
	  ...
	  DbgPrint("AVNetwork || Debug || RtlIpv4StringToAddressW: %x\n", status);
   ...
   )
{
   ...
   if (!NT_SUCCESS(status))
   {
	   DbgPrint("AVNetwork || TLInspectInitDriverObjects error %d\n", status);
	   goto Exit;
   }
   ...
   if (!NT_SUCCESS(status))
   {
	   DbgPrint("AVNetwork || WdfDriverOpenParametersRegistryKey error %d\n", status);
      goto Exit;
   }
   ...
   if (!NT_SUCCESS(status))
   {
	  ...
	  DbgPrint("AVNetwork || TLInspectLoadConfig error %d\n", status);
      goto Exit;
   }
   ...
   {
      status = STATUS_DEVICE_CONFIGURATION_ERROR;
	  DbgPrint("AVNetwork || configInspectRemoteAddrV4 error %d\n", STATUS_DEVICE_CONFIGURATION_ERROR);
      goto Exit;
   }
   ...
   if (!NT_SUCCESS(status))
   {
	   DbgPrint("AVNetwork || FwpsInjectionHandleCreate error %d\n", status);
      goto Exit;
   }
   ...
   if (!NT_SUCCESS(status))
   {
	   DbgPrint("AVNetwork || TLInspectRegisterCallouts error %d\n", status);
      goto Exit;
   }
   ...
   if (!NT_SUCCESS(status))
   {
	   DbgPrint("AVNetwork || PsCreateSystemThread error %d\n", status);
      goto Exit;
   }
