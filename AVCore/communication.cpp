...
--*/
{
	...
	scanThreadCtxes = (PSCANNER_THREAD_CONTEXT)HeapAlloc(GetProcessHeap(), 0, sizeof(SCANNER_THREAD_CONTEXT) * USER_SCAN_THREAD_COUNT);
	...
	for (;;) {
		...
		if (AvMsgEvent == message->Notification.Message) {
			...
			if (FAILED(hr)) {
				...
				fprintf(stderr,
					"[UserScanWorker]: Failed to reply thread handle to the minifilter, %lu\n", hr);

				DisplayError(hr);
				break;
			}
			fprintf(stderr,
				"[UserScanWorker]: Sent reply.\n");
